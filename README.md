...
- **Colour marks the heading, and level marks the depth.** Each channel takes its own step on one soft-orange ramp, spread evenly across however many channels the project has. Its header is a solid orange band with near-black text so it reads as the top of a block; its rows sit on a much lighter wash of the same tone, with the full tone down their left edge, so they read as belonging under it. Blocks are told apart by tone, levels by weight.
- **Status is the other axis, so it uses fill rather than hue:** hollow = ยังไม่เริ่ม, half = กำลังทำ, solid = เสร็จแล้ว, with a dark ring for เลยวันที่ประเมินไว้. The dots hold one fixed ink at every heading — tying them to the channel's tone made a row under the palest channel nearly unreadable, and status has to be equally legible wherever it sits. Channel headers and the project header carry the dots as a tally, so a collapsed project still shows its shape.
...
- Fixed: the on-track Gantt bar was filled `#20262d`, the dark theme's own panel colour, so it was invisible in dark mode. Bars now take their channel's tone, with an outline marking a passed date.
- No SQL for this one.
