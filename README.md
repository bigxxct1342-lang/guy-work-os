...
- **Colour tells you which heading, not which task.** Each channel takes its own step on one soft-orange ramp — a tint behind its header, a thick accent down its left edge, carried onto its rows and its Gantt bar so any row is traceable to the heading it belongs to. With a dozen channels open, the thing you need to tell apart is the block, not the row.
- **Status is the other axis, so it uses fill rather than hue:** hollow = ยังไม่เริ่ม, half = กำลังทำ, solid = เสร็จแล้ว, with a dark ring for เลยวันที่ประเมินไว้. The dots keep one fixed ink at every heading — tying them to the channel's tone made a row under the palest channel nearly unreadable, and status has to be equally legible wherever it sits. Channel headers and the project header carry the dots as a tally, so a collapsed project still shows its shape.
